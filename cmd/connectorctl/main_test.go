...
func TestCatalogLoadsRepositoryManifests(t *testing.T) {
	...
	require.Equal(t, []string{"gmail", "google-sheets", "http", "openai"}, []string{manifests[0].Metadata.Name, manifests[1].Metadata.Name, manifests[2].Metadata.Name, manifests[3].Metadata.Name})
...
func TestReleaseWorkflowIsGeneratedFromSortedCatalog(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, []string{"google/gmail", "google/spreadsheet", "http", "openai"}, []string{entries[0].Slug, entries[1].Slug, entries[2].Slug, entries[3].Slug})
	...
	require.NoError(t, err)
	require.Contains(t, string(content), "          - google/gmail\n          - google/spreadsheet\n          - http\n          - openai\n")
...
func TestGeneratedConnectorsAreCurrent(t *testing.T) {
	for _, manifest := range []string{"http", "openai", "google/gmail", "google/spreadsheet"} {
