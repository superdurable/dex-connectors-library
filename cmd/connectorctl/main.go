...
func connectorReleaseCatalog(root string) ([]connectorCatalogEntry, error) {
	...
	err := filepath.WalkDir(root, func(path string, entry fs.DirEntry, walkErr error) error {
		...
		entries = append(entries, connectorCatalogEntry{
			Slug:         slug,
			ConnectorID:  manifest.Metadata.Name,
...
func generateReleaseWorkflow(entries []connectorCatalogEntry) []byte {
	...
	var releaseDisplayNames strings.Builder
	for _, entry := range entries {
		fmt.Fprintf(&options, "          - %s\n", entry.Slug)
		fmt.Fprintf(&releaseDisplayNames, "            %s) RELEASE_DISPLAY_NAME=%s ;;\n", shellSingleQuote(entry.Slug), shellSingleQuote(entry.DisplayName))
	}
    ...
    steps:
      ...
      - name: Create connector release
        env:
          ...
          RELEASE_VERSION: ${{ steps.plan.outputs.version }}
        run: |
          case "${{ inputs.connector }}" in
` + releaseDisplayNames.String() + `            *) echo "Unknown connector: ${{ inputs.connector }}" >&2; exit 1 ;;
          esac
      ...
      - name: Verify the published Connector with latest stable Dex CLI and Web
        ...
        run: make test-dex-compat-released
