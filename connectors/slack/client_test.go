...
func TestPostThreadReplyUsesBotTokenThreadAndStableClientMessageID(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		_, _ = response.Write([]byte(`{"ok":true,"channel":"C123","ts":"2.0","message":{"ts":"2.0","thread_ts":"1.0","user":"UBOT","text":"处理结束"}}`))
	}))
	defer server.Close()
	client := newSlackClient(t, server.URL)
	result, err := connector.RunMutation(newSlackDexContext("reply-complete"), client.PostThreadReply(), slackConnection, slack.PostThreadReplyInput{ChannelID: "C123", ThreadTimestamp: "1.0", Text: "处理结束"})
	require.NoError(t, err)
